...
import (
	...
	"github.com/threefoldtech/zoslight/pkg/cache"
...
func (g *gatewayModule) ensureGateway(ctx context.Context, forceResstart bool) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "couldn't get traefik service status")
	}
	if exists {
		path, name, err := g.traefikBinary(ctx, z)
		if err != nil {
			return "", errors.Wrap(err, "failed to get old traefik binary path")
		}
		if path != g.binPath {
			if err := z.StopWait(10*time.Second, traefikService); err != nil {
				return "", errors.Wrap(err, "failed to stop old traefik")
			}
			running = false
		...
		if !running {
			if err := z.Forget(traefikService); err != nil {
				return "", errors.Wrap(err, "failed to forget old traefik")
			}
		}
	}
...
func (g *gatewayModule) verifyDomainDestination(ctx context.Context, domain string) error {
	...
	interfaces, err := networker.Interfaces(ctx, types.DefaultBridge, "")
	if err != nil {
		return err
	}
	zosIfIPs := interfaces.Interfaces[types.DefaultBridge].IPs

	var zosIPsList []string
	for _, ip := range zosIfIPs {
		zosIPsList = append(zosIPsList, ip.IP.String())
	}
	...
	for _, ip := range ips {
		if slices.Contains(zosIPsList, ip) {
			return nil
		}
	}
...
func (g *gatewayModule) SetNamedProxy(wlID string, config zos.GatewayNameProxy) (string, error) {
	...
	fqdn := fmt.Sprintf("%s.%s", config.Name, domain)

	gatewayTLSConfig := TlsConfig{
		CertResolver: dnsCertResolver,
		Domains: []Domain{
			{
				Sans: []string{fmt.Sprintf("*.%s", domain)},
			},
		},
	}
...
func (g *gatewayModule) SetFQDNProxy(wlID string, config zos.GatewayFQDNProxy) error {
	...
	if err := g.verifyDomainDestination(ctx, config.FQDN); err != nil {
